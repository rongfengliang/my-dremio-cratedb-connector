...
import com.dremio.exec.store.jdbc.dialect.CrateDBDialect;
...
public class CrateDBConf extends AbstractArpConf<CrateDBConf> {
   private static final String ARP_FILENAME = "arp/implementation/cratedb-arp.yaml";
   private static final CrateDBDialect CRATE_DB_ARP_DIALECT = (CrateDBDialect)AbstractArpConf.loadArpFile(ARP_FILENAME, CrateDBDialect::new);
   ...
   private CloseableDataSource newDataSource(CredentialsService credentialsService) throws SQLException {
      ...
      String credsFromCredentialsService = null;
      if (!Strings.isNullOrEmpty(this.secretResourceUrl)) {
         try {
            credsFromCredentialsService = credentialsService.lookup(this.secretResourceUrl);
         } catch (CredentialsException var5) {
      ...
      return DataSources.newGenericConnectionPoolDataSource("org.postgresql.Driver", this.toJdbcConnectionString(), this.username, credsFromCredentialsService != null ? credsFromCredentialsService : this.password, properties, DataSources.CommitMode.FORCE_MANUAL_COMMIT_MODE, this.maxIdleConns, (long)this.idleTimeSec);
   }
   ...
   public CrateDBDialect getDialect() {
      return CRATE_DB_ARP_DIALECT;
   }

   @VisibleForTesting
   public static CrateDBDialect getDialectSingleton() {
      return CRATE_DB_ARP_DIALECT;
   }
}
